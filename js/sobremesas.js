document.addEventListener('DOMContentLoaded', () => {
  ...
  function atualizarRodape() {
    ...
    document.getElementById('valor-total').textContent = `Total em dinheiro: R$ ${pedido.reduce((s,p)=>s+p.preco,0).toFixed(2)}`;
  }
  ...
  menu.forEach((item, idx) => {
    ...
    card.innerHTML = `
      <div class="card-left">
        <h3><strong><em>${item.nome}</em></strong></h3>
        <p class="desc">${item.descricao}</p>
        <div class="observacao">${item.observacao}</div>
        <p class="price">R$ ${item.preco.toFixed(2)}</p>
      </div>
      <div class="card-right"><img src="${item.imagem}" alt="${item.nome}"></div>
    `;
    ...
    card.addEventListener('click', () => {
      ...
      detailView.querySelector('#add-detail').onclick = () => {
        ...
        pedido.push({nome:item.nome,preco:item.preco,obs}); atualizarRodape(); detailView.classList.add('hidden');
      };
  ...
  function agruparPedido() {
    return Object.values(pedido.reduce((m,p)=>{
      const k = p.nome+"||"+p.obs;
      m[k] = m[k]||{...p,qtd:0}; m[k].qtd++;
      return m;
    },{}));
  }

  function renderizarResumo() {
    const lista = document.getElementById('lista-pedido'); lista.innerHTML='';
    let soma=0;
    agruparPedido().forEach(g=>{
      lista.innerHTML += `<li>${g.nome} x${g.qtd} - R$ ${(g.preco*g.qtd).toFixed(2)}${g.obs?`<div style="font-style:italic;margin-left:20px">Obs: ${g.obs}</div>`:''}</li>`;
      soma += g.preco*g.qtd;
    });
    document.getElementById('total-pedido').textContent = soma.toFixed(2);
  }

  finalizarBtn.onclick = () => { renderizarResumo(); summaryView.classList.remove('hidden'); };
