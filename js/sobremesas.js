document.addEventListener('DOMContentLoaded', () => {
  ...
  // … resto do seu código (renderizar cards, detalheItem, finalizarBtn, etc.)
});
  ...
  finalizarBtn.onclick = () => {
    ...
    detailView.querySelectorAll('.actions button').forEach(btn => {
      btn.onclick = () => {
        ...
        const name = pedido[i].nome;
        const obsOriginal = pedido[i].observacao;
  
        // conta quantos já existem daquele nome
        const count = pedido.filter(p => p.nome === name).length;
  
        if (count + delta >= 1 && count + delta <= 10) {
          if (delta > 0) {
            // Só a primeira unidade carrega a observação; as demais não
            const newObs = (obsOriginal && count === 0) ? obsOriginal : '';  // Para não adicionar a observação nas unidades subsequentes
            pedido.push({ nome: name, preco: pedido[i].preco, observacao: newObs });
          } else {
            // Remover o item correto (encontrar com a observação vazia ou com a observação)
            const remIndex = pedido.findIndex(p => p.nome === name && (!p.observacao || p.observacao === ''));
            if (remIndex > -1) pedido.splice(remIndex, 1);
            else {
              const remObs = pedido.findIndex(p => p.nome === name);
              pedido.splice(remObs, 1);
            }
          }
          atualizarResumoRodape();
